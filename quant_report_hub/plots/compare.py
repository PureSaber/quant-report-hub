...
def plot_14_multi_run(ctx: CompareContext) -> list[Path]:
    ...
    for run in ctx.runs:
        ...
        comm_ratio = float(
            port["commission"].sum()
            / max(abs(port["daily_pnl"].sum() + port["commission"].sum()), 1)
        )
        metrics_rows.append(
            {
                "run_id": run.run_id,
    ...
    if len(metrics_rows) >= 2:
        ...
        for row in metrics_rows:
            ax.scatter(
                abs(row["max_dd"]) * 100, row["total_return"] * 100, s=80, label=row["run_id"]
            )
            ax.annotate(
                row["run_id"], (abs(row["max_dd"]) * 100, row["total_return"] * 100), fontsize=8
            )
        ax.set_xlabel("最大回撤 (%)")
        ax.set_ylabel("累计收益率 (%)")
